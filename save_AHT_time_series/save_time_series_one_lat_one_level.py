...
for year in range(2022, 2023, 1):
    ...
    eddy_all_times = sorted_ds.eddy_tot_int.sel(latitude=which_lat, level=which_lev, method='nearest').values
    mmc_all_times = sorted_ds.mmc_tot_int.sel(latitude=which_lat, level=which_lev, method='nearest').values

    np.save('../aht_time_series/one_level/mmc_all_times_' + which_year + '_' + str(which_lat) + '_' + str(which_lev) +'hPa', mmc_all_times)
    np.save('../aht_time_series/one_level/eddy_all_times_' + which_year + '_' + str(which_lat) + '_' + str(which_lev) +'hPa', eddy_all_times)